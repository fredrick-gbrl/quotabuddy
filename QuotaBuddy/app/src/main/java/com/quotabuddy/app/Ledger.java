...
public class Ledger {
    ...
    /** Attivo per l'addebito: come isActive, ma esclude i cicli su cui è stata impostata un'esenzione manuale.
     * Non influisce sul divisore (activeMemberCount), quindi la quota degli altri membri non cambia. */
    public boolean isChargeable(Models.Membership m,LocalDate cycle,java.util.Set<LocalDate> exemptions){return isActive(m,cycle)&&(exemptions==null||!exemptions.contains(cycle));}
    ...
    public Models.MemberSnapshot snapshot(Models.Membership m,LocalDate today){
        ...
        java.util.Set<LocalDate> exemptions=db.exemptions(m.id);
        ...
        while(guard++<360){
            if(isChargeable(m,cycle,exemptions)){
        ...
        // "coperto fino" mostrato all'utente = ultimo giorno della copertura piena, cioè il giorno prima del prossimo addebito non coperto
        // (non la data del ciclo pagato più recente, che rappresenterebbe il mese sbagliato agli occhi dell'utente).
        out.coveredThrough=lastFully;out.coveredThroughEnd=lastFully==null?null:nextCycle(s,lastFully).minusDays(1);out.nextChargeDate=firstUncovered;out.nextChargeCents=firstCharge;out.nextChargeAlreadyCoveredCents=firstCovered;out.nextChargeRemainingCents=firstRemain;
    ...
    public long chargesThrough(Models.Membership m,LocalDate date){Models.Subscription s=db.subscription(m.subscriptionId);if(s==null||date.isBefore(s.startDate))return 0;java.util.Set<LocalDate> exemptions=db.exemptions(m.id);long sum=0;LocalDate cycle=s.startDate;int guard=0;while(!cycle.isAfter(date)&&guard++<360){if(isChargeable(m,cycle,exemptions))sum+=memberShareAt(s,cycle);cycle=nextCycle(s,cycle);}return sum;}
